...
bool MotionManager::Initialize(CM730 *cm730)
{
	...
	for(int id=JointData::ID_R_SHOULDER_PITCH; id<JointData::NUMBER_OF_JOINTS; id++)
	{
      ...
      if(AX12::isAX12(id))
      {
		   if(m_CM730->ReadWord(id, AX12::P_PRESENT_POSITION_L, &value, &error) == CM730::SUCCESS)
		   {
			   MotionStatus::m_CurrentJoints.SetValue(id, value);
			   MotionStatus::m_CurrentJoints.SetEnable(id, true);
...
// End angle estimator add
        ...
        for(int id=JointData::ID_R_SHOULDER_PITCH; id<JointData::NUMBER_OF_JOINTS; id++)
        {
            if(AX12::isAX12(id))
            {
               ...
               {
                   param2[n++] = id;
                   param2[n++] = MotionStatus::m_CurrentJoints.GetCWSlope(id);
                   param2[n++] = MotionStatus::m_CurrentJoints.GetCCWSlope(id);
                   param2[n++] = CM730::GetLowByte(MotionStatus::m_CurrentJoints.GetValue(id) + m_Offset[id]);
                   param2[n++] = CM730::GetHighByte(MotionStatus::m_CurrentJoints.GetValue(id) + m_Offset[id]);
                   joint_num++;
               }
               if(DEBUG_PRINT == true)
                fprintf(stderr, "ID[%d] : %d \n", id, MotionStatus::m_CurrentJoints.GetValue(id));
    ...
    if(m_IsLogging)
    {
        for(int id = 1; id < JointData::NUMBER_OF_JOINTS; id++)
        {
            ...
            if(AX12::isAX12(id))
               m_LogFileStream << MotionStatus::m_CurrentJoints.GetValue(id) << "," << m_CM730->m_BulkReadData[id].ReadWord(AX12::P_PRESENT_POSITION_L) << ",";
        }
