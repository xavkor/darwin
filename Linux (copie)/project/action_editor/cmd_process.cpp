...
void ReadStep(CM730 *cm730)
{
	...
	{
		if(id >= JointData::ID_R_SHOULDER_PITCH && id <= JointData::ID_L_ELBOW_PITCH)
		{
         if(MX28::isMX28(id))
         {
         ...
         if(AX12::isAX12(id))
         {
			   if(cm730->ReadByte(id, AX12::P_TORQUE_ENABLE, &value, 0) == CM730::SUCCESS)
			   {
				   if(value == 1)
				   {
					   if(cm730->ReadWord(id, AX12::P_GOAL_POSITION_L, &value, 0) == CM730::SUCCESS)
						   Step.position[id] = value;
					   else
						   Step.position[id] = Action::INVALID_BIT_MASK;
				   }
...
void DrawPage()
{
	...
	printf( "ID:21(R_ELB_PITCH)[    ]                                                       \n" );//0
	printf( "ID:22(L_ELB_PITCH)[    ]                                                       \n" );//1
	printf( "   PauseTime      [    ]                                                       \n" );//2
...
void SetValue(CM730 *cm730, int value)
{
	...
	if( col == STP7_COL )
	{
		...
		else
		{
         ...
         if(AX12::isAX12(row+1))
         {
			   if(value >= 0 && value <= AX12::MAX_VALUE)
			   {
				   if(!(Step.position[row + 1] & Action::INVALID_BIT_MASK) && !(Step.position[row + 1] & Action::TORQUE_OFF_BIT_MASK))
				   {
					   int error;
					   if(cm730->WriteWord(row + 1, AX12::P_GOAL_POSITION_L, value, &error) == CM730::SUCCESS)
					   {
						   if(!(error & CM730::ANGLE_LIMIT))
						   {
							   Step.position[row + 1] = value;
...
void ToggleTorque(CM730 *cm730)
{
	...
	if(Step.position[id] & Action::TORQUE_OFF_BIT_MASK)
	{
      ...
      if(AX12::isAX12(id))
      {
		   if(cm730->WriteByte(id, AX12::P_TORQUE_ENABLE, 1, 0) != CM730::SUCCESS)
			   return;

		   int value;
		   if(cm730->ReadWord(id, AX12::P_PRESENT_POSITION_L, &value, 0) != CM730::SUCCESS)
			   return;
	...
	else
	{
      ...
      if(AX12::isAX12(id))
      {
		   if(cm730->WriteByte(id, AX12::P_TORQUE_ENABLE, 0, 0) != CM730::SUCCESS)
			   return;
      }
...
void OnOffCmd(CM730 *cm730, bool on, int num_param, int *list)
{
	if(num_param == 0)
	{
		for(int id=JointData::ID_R_SHOULDER_PITCH; id<JointData::NUMBER_OF_JOINTS; id++)
      {
         ...
         if(AX12::isAX12(id))
			   cm730->WriteByte(id, AX12::P_TORQUE_ENABLE, (int)on, 0);
      }
	}
	else
	{
		...
		{
			if(list[i] >= JointData::ID_R_SHOULDER_PITCH && list[i] <= JointData::ID_L_ELBOW_PITCH)
         {
            if(MX28::isMX28(list[i]))
				   cm730->WriteByte(list[i], MX28::P_TORQUE_ENABLE, (int)on, 0);
            if(AX12::isAX12(list[i]))
				   cm730->WriteByte(list[i], AX12::P_TORQUE_ENABLE, (int)on, 0);
         }
		}
	}
...
void GoCmd(CM730 *cm730, int index)
{
	...
	for(id=JointData::ID_R_SHOULDER_PITCH; id<JointData::NUMBER_OF_JOINTS; id++)
	{
      ...
      if(AX12::isAX12(id))
      {
		   if(cm730->ReadWord(id, AX12::P_PRESENT_POSITION_L, &wStartPosition, 0) != CM730::SUCCESS)
		   {
			   PrintCmd("Failed to read position");
			   return;
		   }
