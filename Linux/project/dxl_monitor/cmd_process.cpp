...
const char *GetIDString(int id)
{
	...
	case JointData::ID_R_ELBOW_PITCH:
		return "R_ELBOW_PITCH";

	case JointData::ID_L_ELBOW_PITCH:
		return "L_ELBOW_PITCH";
...
void Dump(CM730 *cm730, int id)
{
	...
	{	
      ...
      if(AX12::isAX12(id))	
      {
		   if(cm730->ReadTable(id, AX12::P_MODEL_NUMBER_L, AX12::P_PUNCH_H, &table[AX12::P_MODEL_NUMBER_L], 0) != CM730::SUCCESS)
		   {
			   printf(" Can not read table!\n");
			   return;
		   }
		   ...
		   printf( "\n" );
		   printf( " [EEPROM AREA]\n" );
		   addr = AX12::P_MODEL_NUMBER_L; value = CM730::MakeWord(table[addr], table[addr+1]);
		   printf( " MODEL_NUMBER            (R) [%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr+1]);
		   addr = AX12::P_VERSION; value = table[addr];
		   printf( " VERSION                 (R) [%.3d]:%5d\n", addr, value);
		   addr = AX12::P_ID; value = table[addr];
		   printf( " ID                     (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_BAUD_RATE; value = table[addr];
		   printf( " BAUD_RATE              (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_RETURN_DELAY_TIME; value = table[addr];
		   printf( " RETURN_DELAY_TIME      (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_CW_ANGLE_LIMIT_L; value = CM730::MakeWord(table[addr], table[addr+1]);
		   printf( " CW_ANGLE_LIMIT         (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr+1]);
		   addr = AX12::P_CCW_ANGLE_LIMIT_L; value = CM730::MakeWord(table[addr], table[addr+1]);
		   printf( " CCW_ANGLE_LIMIT        (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr+1]);
		   addr = AX12::P_HIGH_LIMIT_TEMPERATURE; value = table[addr];
		   printf( " HIGH_LIMIT_TEMPERATURE (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_LOW_LIMIT_VOLTAGE; value = table[addr];
		   printf( " LOW_LIMIT_VOLTAGE      (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_HIGH_LIMIT_VOLTAGE; value = table[addr];
		   printf( " HIGH_LIMIT_VOLTAGE     (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_MAX_TORQUE_L; value = CM730::MakeWord(table[addr], table[addr+1]);
		   printf( " MAX_TORQUE             (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr+1]);
		   addr = AX12::P_RETURN_LEVEL; value = table[addr];
		   printf( " RETURN_LEVEL           (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_ALARM_LED; value = table[addr];
		   printf( " ALARM_LED              (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_ALARM_SHUTDOWN; value = table[addr];
		   printf( " ALARM_SHUTDOWN         (R/W)[%.3d]:%5d\n", addr, value);
		   printf( "\n" );
		   printf( " [RAM AREA]\n" );
		   addr = AX12::P_TORQUE_ENABLE; value = table[addr];
		   printf( " TORQUE_ENABLE          (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_LED; value = table[addr];
		   printf( " LED                    (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_CW_COMPLIANCE_MARGIN; value = table[addr];
		   printf( " CW_COMPLIANCE_MARGIN   (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_CCW_COMPLIANCE_MARGIN; value = table[addr];
		   printf( " CCW_COMPLIANCE_MARGIN  (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_CW_COMPLIANCE_SLOPE; value = table[addr];
		   printf( " CW_COMPLIANCE_SLOPE    (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_CCW_COMPLIANCE_SLOPE; value = table[addr];
		   printf( " CCW_COMPLIANCE_SLOPE   (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_GOAL_POSITION_L; value = CM730::MakeWord(table[addr], table[addr+1]);
		   printf( " GOAL_POSITION          (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr+1]);
		   addr = AX12::P_MOVING_SPEED_L; value = CM730::MakeWord(table[addr], table[addr+1]);
		   printf( " MOVING_SPEED           (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr+1]);
		   addr = AX12::P_TORQUE_LIMIT_L; value = CM730::MakeWord(table[addr], table[addr+1]);
		   printf( " TORQUE_LIMIT           (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr+1]);
		   addr = AX12::P_PRESENT_POSITION_L; value = CM730::MakeWord(table[addr], table[addr+1]);
		   printf( " PRESENT_POSITION       (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr+1]);
		   addr = AX12::P_PRESENT_SPEED_L; value = CM730::MakeWord(table[addr], table[addr+1]);
		   printf( " PRESENT_SPEED          (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr+1]);
		   addr = AX12::P_PRESENT_LOAD_L; value = CM730::MakeWord(table[addr], table[addr+1]);
		   printf( " PRESENT_LOAD           (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr+1]);
		   addr = AX12::P_PRESENT_VOLTAGE; value = table[addr];
		   printf( " PRESENT_VOLTAGE        (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_PRESENT_TEMPERATURE; value = table[addr];
		   printf( " PRESENT_TEMPERATURE    (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_REGISTERED_INSTRUCTION; value = table[addr];
		   printf( " REGISTERED_INSTRUC     (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_MOVING; value = table[addr];
		   printf( " MOVING                 (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_LOCK; value = table[addr];
		   printf( " LOCK                   (R/W)[%.3d]:%5d\n", addr, value);
		   addr = AX12::P_PUNCH_L; value = CM730::MakeWord(table[addr], table[addr+1]);
		   printf( " PUNCH                  (R/W)[%.3d]:%5d (L:0x%.2X H:0x%.2X)\n", addr, value, table[addr], table[addr+1]);
...
void Reset(Robot::CM730 *cm730, int id)
{
	...
	while(1)
	{
      if(MX28::isMX28(id))
		   if(cm730->WriteByte(id, MX28::P_RETURN_DELAY_TIME, 0, 0) == CM730::SUCCESS)
			   break;
      if(AX12::isAX12(id))
		   if(cm730->WriteByte(id, AX12::P_RETURN_DELAY_TIME, 0, 0) == CM730::SUCCESS)
		     break;     
	...
	while(1)
	{
      if(MX28::isMX28(id))
		   if(cm730->WriteByte(id, MX28::P_RETURN_LEVEL, 2, 0) == CM730::SUCCESS)
			   break;
      if(AX12::isAX12(id))
		   if(cm730->WriteByte(id, AX12::P_RETURN_LEVEL, 2, 0) == CM730::SUCCESS)
			   break;
	...
	if(id != CM730::ID_CM)
	{
      ...
      if(MX28::isMX28(id))
      {
		   cwLimit = MX28::MIN_ANGLE;
		   ccwLimit = MX28::MAX_ANGLE;
      }
      if(AX12::isAX12(id))
      {
		   cwLimit = AX12::MIN_ANGLE;
		   ccwLimit = AX12::MAX_ANGLE;
      }
		...
		while(1)
		{
         if(MX28::isMX28(id))
			   if(cm730->WriteWord(id, MX28::P_CW_ANGLE_LIMIT_L, MX28::Angle2Value(cwLimit), 0) == CM730::SUCCESS)
				   break;
         if(AX12::isAX12(id))
			   if(cm730->WriteWord(id, AX12::P_CW_ANGLE_LIMIT_L, AX12::Angle2Value(cwLimit), 0) == CM730::SUCCESS)
				   break;
		...
		{
         if(MX28::isMX28(id))
			   if(cm730->WriteWord(id, MX28::P_CW_ANGLE_LIMIT_L, MX28::Angle2Value(ccwLimit), 0) == CM730::SUCCESS)
				   break;
         if(AX12::isAX12(id))
			   if(cm730->WriteWord(id, AX12::P_CW_ANGLE_LIMIT_L, AX12::Angle2Value(ccwLimit), 0) == CM730::SUCCESS)
				   break;
		...
		while(1)
		{
         if(MX28::isMX28(id))
			   if(cm730->WriteByte(id, MX28::P_HIGH_LIMIT_TEMPERATURE, 80, 0) == CM730::SUCCESS)
				   break;
         if(AX12::isAX12(id))
			   if(cm730->WriteByte(id, AX12::P_HIGH_LIMIT_TEMPERATURE, 80, 0) == CM730::SUCCESS)
				   break;
		...
		while(1)
		{
         if(MX28::isMX28(id))
			   if(cm730->WriteByte(id, MX28::P_LOW_LIMIT_VOLTAGE, 60, 0) == CM730::SUCCESS)
				   break;
         if(AX12::isAX12(id))
			   if(cm730->WriteByte(id, AX12::P_LOW_LIMIT_VOLTAGE, 60, 0) == CM730::SUCCESS)
				   break;
		...
		while(1)
		{
         if(MX28::isMX28(id))
			   if(cm730->WriteByte(id, MX28::P_HIGH_LIMIT_VOLTAGE, 140, 0) == CM730::SUCCESS)
				   break;
         if(AX12::isAX12(id))
			   if(cm730->WriteByte(id, AX12::P_HIGH_LIMIT_VOLTAGE, 140, 0) == CM730::SUCCESS)
				   break;
		...
		while(1)
		{
         if(MX28::isMX28(id))
			   if(cm730->WriteWord(id, MX28::P_MAX_TORQUE_L, MX28::MAX_VALUE, 0) == CM730::SUCCESS)
				   break;
         if(AX12::isAX12(id))
			   if(cm730->WriteWord(id, AX12::P_MAX_TORQUE_L, AX12::MAX_VALUE, 0) == CM730::SUCCESS)
				   break;
		...
		while(1)
		{
         if(MX28::isMX28(id))
			   if(cm730->WriteByte(id, MX28::P_ALARM_LED, 36, 0) == CM730::SUCCESS) // Overload, Overheat
				   break;
         if(AX12::isAX12(id))
			   if(cm730->WriteByte(id, AX12::P_ALARM_LED, 36, 0) == CM730::SUCCESS) // Overload, Overheat
				   break;
		...
		while(1)
		{
         if(MX28::isMX28(id))
			   if(cm730->WriteByte(id, MX28::P_ALARM_SHUTDOWN, 36, 0) == CM730::SUCCESS) // Overload, Overheat
				   break;
         if(AX12::isAX12(id))
			   if(cm730->WriteByte(id, AX12::P_ALARM_SHUTDOWN, 36, 0) == CM730::SUCCESS) // Overload, Overheat
				   break;
...
void Write(Robot::CM730 *cm730, int id, int addr, int value)
{
   ...
   if(AX12::isAX12(id))
	   if(addr == AX12::P_RETURN_DELAY_TIME || addr == AX12::P_RETURN_LEVEL)
	   {
		   printf( " Can not change this address[%d]\n", addr);
		   return;
	   }
	...
	else
	{
      ...
      if(AX12::isAX12(id))
      {
		   ...
		   else if(addr == AX12::P_HIGH_LIMIT_TEMPERATURE
               || addr == AX12::P_LOW_LIMIT_VOLTAGE
               || addr == AX12::P_HIGH_LIMIT_VOLTAGE
               || addr == AX12::P_ALARM_LED
               || addr == AX12::P_ALARM_SHUTDOWN
               || addr == AX12::P_TORQUE_ENABLE
               || addr == AX12::P_LED
               || addr == AX12::P_CW_COMPLIANCE_MARGIN
               || addr == AX12::P_CCW_COMPLIANCE_MARGIN
               || addr == AX12::P_CW_COMPLIANCE_SLOPE
               || addr == AX12::P_CCW_COMPLIANCE_SLOPE
			      || addr == AX12::P_LED
			      || addr == AX12::P_LED)
		   {
			   res = cm730->WriteByte(id, addr, value, &error);
